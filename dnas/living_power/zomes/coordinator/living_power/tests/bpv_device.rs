...
use living_power::bpv_device::UpdateBpvDeviceInput;
...
async fn create_bpv_device_test() {
    ...
    let entry: BpvDevice = record.entry().to_app_option().unwrap().unwrap();
    assert!(entry.eq(&sample));
}


#[tokio::test(flavor = "multi_thread")]
async fn create_and_read_bpv_device() {
    // Use prebuilt dna file
    let dna_path = std::env::current_dir()
        .unwrap()
        .join(std::env::var("DNA_PATH").expect("DNA_PATH not set, must be run using nix flake check"));
    ...
    let get_record: Option<Record> = conductors[1]
        .call(&bob_zome, "get_original_bpv_device", record.signed_action.action_address().clone())
        .await;
...
#[tokio::test(flavor = "multi_thread")]
async fn create_and_update_bpv_device() {
    // Use prebuilt dna file
    let dna_path = std::env::current_dir()
        .unwrap()
        .join(std::env::var("DNA_PATH").expect("DNA_PATH not set, must be run using nix flake check"));
    ...
    let sample_1 = sample_bpv_device_1(&conductors[0], &alice_zome).await;
    
    // Alice creates a BpvDevice
    let record: Record = create_bpv_device(&conductors[0], &alice_zome, sample_1.clone()).await;
    ...
    let sample_2 = sample_bpv_device_2(&conductors[0], &alice_zome).await;
    let input = UpdateBpvDeviceInput {
      previous_bpv_device_hash: original_action_hash.clone(),
      updated_bpv_device: sample_2.clone(),
    };
    
    // Alice updates the BpvDevice
    let update_record: Record = conductors[0]
        .call(&alice_zome, "update_bpv_device", input)
        .await;
        
    let entry: BpvDevice = update_record.entry().to_app_option().unwrap().unwrap();
    ...
    let get_record: Option<Record> = conductors[1]
        .call(&bob_zome, "get_latest_bpv_device", original_action_hash.clone())
        .await;
  
    assert_eq!(update_record, get_record.unwrap());
    
    let input = UpdateBpvDeviceInput {
      previous_bpv_device_hash: update_record.signed_action.hashed.hash.clone(),
      updated_bpv_device: sample_1.clone(),
    };
...
#[tokio::test(flavor = "multi_thread")]
async fn create_and_delete_bpv_device() {
    // Use prebuilt dna file
    let dna_path = std::env::current_dir()
        .unwrap()
        .join(std::env::var("DNA_PATH").expect("DNA_PATH not set, must be run using nix flake check"));
    ...
    let delete_action_hash: ActionHash = conductors[0]
        .call(&alice_zome, "delete_bpv_device", original_action_hash.clone())
        .await;
    ...
    let deletes: Vec<SignedActionHashed> = conductors[1]
        .call(&bob_zome, "get_all_deletes_for_bpv_device", original_action_hash.clone())
        .await;
