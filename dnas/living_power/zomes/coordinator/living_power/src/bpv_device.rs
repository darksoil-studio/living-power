...
pub fn create_bpv_device(bpv_device: BpvDevice) -> ExternResult<Record> {
    ...
    let record = get(bpv_device_hash.clone(), GetOptions::default())?
        .ok_or(
            wasm_error!(
                WasmErrorInner::Guest("Could not find the newly created BpvDevice"
                .to_string())
            ),
        )?;
...
#[hdk_extern]
pub fn get_original_bpv_device(
    original_bpv_device_hash: ActionHash,
) -> ExternResult<Option<Record>> {
    let Some(details) = get_details(original_bpv_device_hash, GetOptions::default())?
    else {
        return Ok(None);
    };
    match details {
...
#[hdk_extern]
pub fn get_all_revisions_for_bpv_device(
    original_bpv_device_hash: ActionHash,
) -> ExternResult<Vec<Record>> {
    let Some(Details::Record(details)) = get_details(
        original_bpv_device_hash,
        GetOptions::default(),
    )? else {
        return Ok(vec![]);
    };
    ...
    for update in details.updates {
        let mut update_records = get_all_revisions_for_bpv_device(
...
pub fn update_bpv_device(input: UpdateBpvDeviceInput) -> ExternResult<Record> {
    let updated_bpv_device_hash = update_entry(
        input.previous_bpv_device_hash,
        &input.updated_bpv_device,
    )?;
    let record = get(updated_bpv_device_hash.clone(), GetOptions::default())?
        .ok_or(
            wasm_error!(
                WasmErrorInner::Guest("Could not find the newly updated BpvDevice"
                .to_string())
            ),
        )?;
    Ok(record)
...
#[hdk_extern]
pub fn delete_bpv_device(
    original_bpv_device_hash: ActionHash,
) -> ExternResult<ActionHash> {
    let path = Path::from("all_bpv_devices");
    let links = get_links(
        GetLinksInputBuilder::try_new(path.path_entry_hash()?, LinkTypes::AllBpvDevices)?
            .build(),
    )?;
    for link in links {
        if let Some(hash) = link.target.into_action_hash() {
            if hash.eq(&original_bpv_device_hash) {
...
pub fn get_oldest_delete_for_bpv_device(
    ...
    let Some(mut deletes) = get_all_deletes_for_bpv_device(original_bpv_device_hash)?
    else {
        return Ok(None);
    };
    deletes
