...
import './measure-collection-summary.js';

/**
 * @element measure-collections-for-bpv-device
 */
@localized()
@customElement('measure-collections-for-bpv-device')
export class MeasureCollectionsForBpvDevice extends SignalWatcher(LitElement) {

  /**
   * REQUIRED. The BpvDeviceHash for which the MeasureCollections should be fetched
   */
  ...
  renderList(hashes: Array<ActionHash>) {
    if (hashes.length === 0) 
      return html` <div class="column center-content" style="gap: 16px;">
        ...
        ></sl-icon>
        <span class="placeholder">${msg("No measure collections found for this bpv device")}</span>
      </div>`;

    return html`
      <div style="display: flex; flex-direction: column">
        ${hashes.map(hash =>
          html`<measure-collection-summary .measureCollectionHash=${hash}></measure-collection-summary>`
        )}
      </div>
    `;
  }
  ...
  render() {
    const map = this.livingPowerStore.bpvDevices.get(this.bpvDeviceHash).measureCollections.live.get();
