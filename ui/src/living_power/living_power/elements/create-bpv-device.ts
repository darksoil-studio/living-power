...
export class CreateBpvDevice extends SignalWatcher(LitElement) {
	...
	async createBpvDevice(fields: Partial<BpvDevice>) {
		if (this.arduinoSerialNumber === undefined)
			throw new Error(
				'Cannot create a new Bpv Device without its arduino_serial_number field',
			);
		...
		try {
			...
			const record: EntryRecord<BpvDevice> =
				await this.livingPowerStore.client.createBpvDevice(bpvDevice);
