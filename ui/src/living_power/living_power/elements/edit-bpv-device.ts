...
export class EditBpvDevice extends SignalWatcher(LitElement) {
	...
	async firstUpdated() {
		const currentRecord = await toPromise(
			this.livingPowerStore.bpvDevices.get(this.bpvDeviceHash).latestVersion,
		);
		setTimeout(() => {
	...
	async updateBpvDevice(
		currentRecord: EntryRecord<BpvDevice>,
		fields: Partial<BpvDevice>,
	) {
		...
		try {
			...
			this.dispatchEvent(
				new CustomEvent('bpv-device-updated', {
					composed: true,
					bubbles: true,
					detail: {
